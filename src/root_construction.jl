...
function grow!(r::Root{T, U}, re::RootEdge{T, U}, rg::RootGraph{T, U}) where {T, U}
    ...
    if is_upstream
        pushfirst!(V(r), getrootvertex(rg, new_vertex))
    else
        push!(V(r), getrootvertex(rg, new_vertex))
    end
...
function separate_root_systems(rg::RootGraph, re_classification_dict::Dict, rs::Vector{<:Root})
